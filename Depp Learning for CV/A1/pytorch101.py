...
def multiples_of_ten(start: int, stop: int) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    first = (start + 9) // 10 * 10  # Arredonda 'start' para o próximo múltiplo de 10
    if first > stop:  # Se não há múltiplos de 10 no intervalo, retorna tensor vazio
        return torch.empty((0,), dtype=torch.float64)
    
    # Criar um tensor com múltiplos de 10 usando torch.arange
    x = torch.arange(first, stop + 1, step=10, dtype=torch.float64)
    ##########################################################################
    #                 = torch.arange(start, stop, dtype=torch.float64)           END OF YOUR CODE                            #
    ##########################################################################
    return x
...
def slice_indexing_practice(x: Tensor) -> Tuple[Tensor, Tensor, Tensor, Tensor]:
    ...
    # Para pegar as colunas impares, comecamos na coluna um e vamos de 2 em 2
...
def reverse_rows(x: Tensor) -> Tensor:
    ...
    # Utiliza o array de índices para reordenar as linhas de x
    y = x[indices]
...
def take_one_elem_per_col(x: Tensor) -> Tensor:
    ...
    # Acessando os elementos com uma única operação de indexação:
...
def make_one_hot(x: List[int]) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    max_value = max(x) + 1  # C = 1 + max(x)
    y = torch.zeros((len(x), max_value), dtype=torch.float32)
    
    # Cria um tensor para todas as linhas a serem percorridas
    row_indices = torch.arange(len(x))
    
    # Cria um tensor para as colunas a partir da lista x
    col_indices = torch.tensor(x)
    
    # Para cada linha i, define o elemento na coluna x[i] como 1
...
def sum_positive_entries(x: Tensor) -> Tensor:
    ...
    # O uso da mascara faz com que apenas as entradas True facam parte da operacao de soma
    pos_sum = x[x >= 0].sum()
...
def reshape_practice(x: Tensor) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    left = x[:12].reshape(3, 4)
    # Reestrutura a segunda metade (elementos 12 a 23) em um tensor (3, 4)
    right = x[12:].reshape(3, 4)
    # Concatena as duas partes ao longo das colunas (dimensão 1)
...
def zero_row_min(x: Tensor) -> Tensor:
    ...
    # Para cada linha, obtém o índice da coluna com o menor valor
    col_indices = torch.argmin(y, dim=1)
    
    # Define o menor valor de cada linha como 0
...
def batched_matrix_multiply_loop(x: Tensor, y: Tensor) -> Tensor:
    ...
    # Replace "pass" statement with your code
    B, N, M = x.shape
    # Pré-aloca o tensor resultado com o mesmo tipo e device de x
    z = torch.empty((B, N, y.shape[2]), dtype=x.dtype, device=x.device)
    
    # Loop sobre a dimensão do batch
    for i in range(B):
...
def normalize_columns(x: Tensor) -> Tensor:
    ...
    # Calcula a média de cada coluna: shape (1, N) para facilitar broadcasting
    mu = x.sum(dim=0, keepdim=True) / M

    # Calcula a variância de cada coluna e depois o desvio padrão
    sigma = (((x - mu) ** 2).sum(dim=0, keepdim=True) / M).sqrt()

    # Normaliza as colunas: subtrai a média e divide pelo desvio padrão
    y = (x - mu) / sigma
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
