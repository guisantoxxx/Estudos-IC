...
def svm_loss_naive(
    ...
    # Derivada do termo de regularização: d/dW (reg * sum(W*W)) = 2 * reg * W
    dW += 2 * reg * W
...
def svm_loss_vectorized(
    ...
    # Replace "pass" statement with your code
    N = X.shape[0]
    scores = X.mm(W)  # (N, C) - Pontuações para todas as classes
    correct_scores = scores[torch.arange(N), y]  # (N,) - Pontuações das classes corretas
    
    # Margens para todas as classes (delta = 1)
    margins = scores - correct_scores.view(-1, 1) + 1.0  # (N, C)
    margins[torch.arange(N), y] = 0.0  # Zera margens da classe correta
    
    # Calcula a perda (média dos valores positivos + regularização)
    loss = torch.clamp(margins, min=0).sum() / N
    loss += reg * torch.sum(W * W)  # Termo de regularização
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
    ...
    #############################################################################
    # Replace "pass" statement with your code
    # Máscara para margens positivas (ignora classe correta)
    mask = (margins > 0).float()  # (N, C)
    
    # Quantidade de margens positivas por exemplo
    sum_coeff = mask.sum(dim=1)  # (N,)
    
    # Matriz one-hot das classes corretas
    one_hot_y = torch.zeros_like(scores)
    one_hot_y.scatter_(1, y.view(-1, 1), 1.0)  # (N, C)
    
    # Coeficientes finais para o gradiente
    coeff_total = mask - sum_coeff.view(-1, 1) * one_hot_y
    
    # Gradiente da perda de dados
    dW = X.t().mm(coeff_total) / N  # (D, C)
    
    # Gradiente da regularização (derivada de reg*sum(W^2) = 2*reg*W)
    dW += 2 * reg * W
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
...
def sample_batch(
    ...
    # Seleciona os exemplos e labels correspondentes
    X_batch = X[indices]
    y_batch = y[indices]
    #########################################################################
    #                       END OF YOUR CODE                                #
